...
class LNNQAgent(ReinforcementAgent):
    def __init__(
        ...
        **kwargs
        ...
        alpha = 0.2
        gamma = 0.8
        super().__init__(epsilon=epsilon, alpha=alpha, gamma=gamma, **kwargs)
    ...
    def computeActionFromQValues(self, state):
        ...
        if len(actions) == 0:
            # print('Num of legalActions is 0 in compute action from q values')
            return None
        ...
        for action in best_actions:
            ...
            if self.lnn.is_action_safe(close_ghosts['num_close_ghosts']):
                return action
    ...
    def computeValueFromQValues(self, state):
        ...
        if len(actions) == 0:
            # print('Num of legalActions is 0 in compute value from q values')
            return 0.0
    ...
    def getAction(self, state):
        ...
        if len(legalActions) == 0:
            # print('Num of legalActions is 0 in get action')
            return None
    ...
    def final(self, state):
        ...
        if self.episodesSoFar == self.numTraining:
            ...
            print(f'Length of q table: {len(self.values)}')
...
class LNNApproximateQAgent(LNNQAgent):
    ...
    def getQValue(self, state, action):
        ...
        features = self.feature_extractor.getFeatures(state, action)
        value = 0
    ...
    def final(self, state):
        ...
        if self.episodesSoFar == self.numTraining:
            # you might want to print your weights here for debugging
            print(f'These are the Approx Q Agent weights: {self.weights}')